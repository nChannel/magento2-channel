function InsertProductSimple(ncUtil, channelProfile, flowContext, payload, callback) {
  const nc = require("../util/ncUtils");
  const referenceLocations = ["productBusinessReferences"];
  const stub = new nc.Stub("InsertProductSimple", referenceLocations, ...arguments);
  ...
  async function insertProduct() {
    logInfo("Inserting new product record...");
    return await stub.request.post({ url: "/V1/products", body: stub.payload.doc });
  }
